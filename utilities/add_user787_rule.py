...
def create_routing_rule_for_user787():
    """Create a routing rule for calls from user787 to user999."""
    try:
        # Create gRPC channel
        channel = grpc.insecure_channel('localhost:50051')
        client = RoutingRuleServiceStub(channel)
        
        print("📝 Creating routing rule for user787 -> user999...")
        
        # Create routing conditions
        conditions = b2bua_pb2.RoutingConditions(
            from_uri_regex="^sip:user787@.*",  # Match calls from user787
        ...
        rule = b2bua_pb2.RoutingRule(
            rule_id="test-rule-user787",
            name="Test Rule for user787",
            description="Route calls from user787 to user999 for session limits testing",
            priority=100,
...
def main():
    print("🔧 Adding routing rule for user787\n")
    
    if not create_routing_rule_for_user787():
        return 1
    
    print("\n✅ Routing rule for user787 created successfully!")
    return 0
